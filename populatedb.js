...
function supplierCreate(name, cnpj, city, state, cb){
  supplierDetail = { name: name, cnpj: cnpj, state: state };
  if(city != false) supplierDetail.city = city;
...
function productCreate(name, description, quantity, price, category, supplier, cb){
  productDetail = { name: name, price: price, category: category, supplier: supplier };
  if(description != false) productDetail.description = description;
  if(quantity != false) productDetail.quantity = quantity;
...
function createCategory(cb){
  async.series([
    ...
    function(callback){
      categoryCreate('Jogos', callback);
    },
    function(callback){
      categoryCreate('Console', callback);
    }
  ], cb);
}
...
function createSupplier(cb){
  async.series([
    function(callback){
      supplierCreate('Americanas', 69540296000130, false, 'SP', callback);
    },
    function(callback){
      supplierCreate('Kabum', 48610732000120, 'Itapevi', 'SP', callback);
    },
    function(callback){
      supplierCreate('Samsung', 72719140000125, 'São José do Rio Preto', 'SP', callback);
    },
    function(callback){
      supplierCreate('Casas Bahia', 89909587000160, 'Macaé', 'RJ', callback);
    }
  ], cb);
}
  ...
  async.parallel([
    function(callback){
      productCreate(
        ...
        'Smartphone Xiaomi Redmi Note 11 Dual 128gb 6gb Ram - Graphite Gray/cinza - Global Tela AMOLED Mergulhe em um mundo de maravilhas A tela retroiluminada oferece o que há de mais moderno em brilho, contraste, calibração de cores e resolução. Taxa de atualização de 90Hz Ecrã e ecrã tátil sem falhas.',
        100,
        1308.80,
        categories[0],
        suppliers[1],
    ...
    function(callback){
      productCreate(
        'Teclado Mecânico Gamer HyperX Alloy MKW100, RGB, Switch Red, Full Size, Layout US',
    ...
    function(callback){
      productCreate(
        'Console Microsoft Xbox Series S, 512GB, Branco',
        'Xbox Series S, o menor e mais elegante de todos os tempos. Com taxas de quadros mais altas, tempos de carregamento mais rápidos e mundos mais ricos e dinâmicos, o Xbox Series S oferece desempenho e velocidade de última geração em um formato compacto e totalmente digital.',
        2,
        2500.00,
        categories[3],
        suppliers[3],
